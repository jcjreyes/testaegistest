...
const Standard = ({ header, data, link, buttonLabel }) => {
    const count = data && data.filter((i) => i.status === "pending").length;
    const newRequests =
        data &&
        data.filter(
            (i) =>
                moment(i.date_submitted).format("YYYY-MM-DD") ===
                moment().format("YYYY-MM-DD") && i.status === "pending"
        ).length;
    ...
    return (
        <Card>
            ...
            <InfoWrapper>
                <InfoText>{newRequests} New Requests</InfoText>
            </InfoWrapper>
            <MainInfo>
