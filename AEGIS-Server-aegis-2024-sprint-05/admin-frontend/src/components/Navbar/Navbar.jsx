    ...
    <Nav>
        <NavContainer>
            ...
            ?
                <>
                    <NavProfile>
                        <Avatar name={/*user.user.first_name*/"Joshua Taningco"} size="52" round={true} color="#FDFEFF" fgColor="#001196" />
                        <NavProfileContent>
                            <NavProfileName>{/*user.user.first_name} {user.user.last_name*/"Joshua Taningco"}</NavProfileName>
                            <NavProfilePosition>Admin</NavProfilePosition>
                        </NavProfileContent>
                    </NavProfile>
                    <NavMenu>
                        {navLinks.map((link, index) => {
                            return (
                                link.name == "Sign Out"
                                ?
                                <Link to={link.route} style={{"text-decoration": "none"}} onClick={e => (localStorage.clear(), window.location.href="/login")}>
                                <NavItem key={index}>
                                    <NavItemIcon src={link.icon} />
                                    <NavLink to="/logout" active={slug === link.route}>{link.name}</NavLink>
                                </NavItem>
                                </Link>
                                :
                                <Link to={link.route} style={{"text-decoration": "none"}}>
                                <NavItem key={index} to={link.route} className={slug === link.route ? "activeNav" : ""}>
                                    <NavItemIcon src={link.icon} />
