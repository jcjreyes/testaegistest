...
class AccountsTest(APITestCase):
    def setUp(self):
        ...
        academic_information = AcademicInformation.objects.create(
            ...
            primary_course=data.get("primary_course"),
            secondary_course=data.get("secondary_course"),
        )
        self.pre_created_user = user
        self.user_url = f"/api/users/{user.id}/"

    def test_should_create_account(self):
        """
        Ensure we can create an account object
        """
        data = self.data
